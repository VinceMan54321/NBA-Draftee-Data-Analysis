...
```{r height-visualization}
ggplot(data = draft_data, aes(x = season, y = player_height)) +
  geom_histogram(stat = 'identity') + labs(x = "Season", y = "Height (cm)",
  title = "Height vs Season of NBA rookies since 1996",
  subtitle = "faceted by every ten picks") + facet_wrap(~draft_group)
```
```{r}
ggplot(data = draft_data, aes(x = season, y = player_height)) +

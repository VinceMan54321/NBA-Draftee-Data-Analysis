...
Taking a closer look at our data set by year, we can see that although there are some outliers, consistent data on players across the league began to be recorded around the year 1981. For this reason, we will pick 1996 as the starting year for our analysis and filter out any recorded data from before 1996. We will also cap our analysis at before the 2019 season, as that season was ongoing at the time of upload, so metrics such as gp, pts, reb, etc. would not be complete.

This data set is biased towards the offensive side of the ball, as it lacks defensive statistics such as blocks or steals. Therefore, the contributions of players focused on the defensive side may be neglected.  
...
```
Our p-value was about `r p_val`, and so we reject the null hypothesis at the a = 0.05 level. This is further backed by our confidence interval, [`r round(ci1$lower, 3)`, `r round(ci1$upper, 3)`], and our null distribution graph, which shows that our observed difference is outside the confidence interval. There is sufficient evidence to suggest that a larger proportion guards of guards were picked in the first round compared to the second round, meaning that skilled guards in general are likely more valued than forwards and centers.
...
ggplot(data = null_dist, aes(x = stat)) +
  ...
  labs(x = "Mean Year", y = "Count",
       ...
       subtitle = "2,500 bootstrap simulations with 95% CI shown")
```
Our confidence interval was [`r round(ci2$lower, 3)`, `r round(ci2$upper, 3)`]. Our null distribution graph shows that our observed difference in means is approximately in the middle of the confidence interval, which shows that we will likely not be able to reject our null hypothesis. 

This is proven by our p-value, `r p_val`, which is much greater than 0.05, so we fail to reject our null hypothesis. Therefore, there is no sufficient evidence that the mean year of guards who were drafted is either different or not different than non guards who were drafted. In the context of our research question, there is not sufficient evidence to prove that there has been a shift in preferences to prefer picking guards during the first draft round. 
...
Using a two-sided t-test with a test statistic of `r round(ttest$statistic, 3)` and `r round(ttest$t_df, 0)` degrees of freedom, we found a p-value of `r round(ttest$p_value, 3)` and a confidence interval of [`r round(ttest$lower_ci, 3)`, `r round(ttest$upper_ci, 3)`]. We are 95% confident that the mean number of rebounds for players is between `r round(ttest$lower_ci, 3)` to `r round(ttest$upper_ci, 3)`. If we were to repeatedly and independently sample from the original population and construct 95% CIs in the same way, we would expect 95% of such intervals to truly contain the population mean.
...
Using a two-sided t-test with a test statistic of `r round(temp$statistic, 3)` and `r round(temp$t_df, 0)` degrees of freedom, we found a p-value of `r round(temp$p_value, 3)` and a confidence interval of [`r round(temp$lower_ci, 3)`, `r round(temp$upper_ci, 3)`]. We are 95% confident that the mean number of rebounds for players from the early 2000s (years 2000-2006) is `r round(temp$lower_ci, 3)` to `r round(temp$upper_ci, 3)`. If we were to repeatedly and independently sample from the original population and construct 95% CIs in the same way, we would expect 95% of such intervals to truly contain the population mean.
